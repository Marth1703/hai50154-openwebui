...
def clean_text(tag):
    # Remove hyperlinks and reference tags
...
def main(url=None):
    ...
    # Group sections by header level and write to a single markdown file
    md_lines = []
    for header, content in sections:
        ...
        md_lines.append(f"{'#' * level} {header}\n")
        if content.strip():
            md_lines.append(f"{content.strip()}\n")

    # Write all sections to a single markdown file
    md_path = os.path.join(out_dir, "sections.md")
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(md_lines))

    print(f"Saved markdown file with {len(sections)} sections to ./{out_dir}/sections.md")
